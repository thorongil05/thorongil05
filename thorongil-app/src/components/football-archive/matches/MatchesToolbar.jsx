  ...
  return (
    <div className="p-4 border-b border-slate-800 space-y-3">
      ...
      <div className="flex gap-2 flex-wrap">
        <select value={selectedRound || "All"} onChange={(e) => setSelectedRound(e.target.value)} disabled={!selectedEdition} className={selectCls}>
        ...
        {isFiltered(selectedRound, selectedTeamId, sortBy) && (
          <button onClick={handleResetFilters} className="px-3 py-2 rounded-lg text-sm text-slate-400 border border-slate-700 hover:border-slate-500 hover:text-white transition-all">
            ↺ Reset
          </button>
        )}
      </div>
